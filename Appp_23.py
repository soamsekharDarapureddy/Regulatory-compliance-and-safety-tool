...
.big-banner{background:var(--accent); padding:16px 22px; border-radius:14px; margin-bottom:18px;}
.big-banner h1{color:#fff; font-size:1.9em; margin:0 0 4px 0;}
.big-banner p{color:#eaf4ff; margin:0; font-weight:500;}
.gem-panel{background:var(--panel); border-radius:14px; padding:16px 14px; box-shadow:0 2px 16px var(--shadow);}
...
<div class="big-banner">
  <h1>🛡️ E‑Bike Regulatory Compliance & Safety Checker</h1>
  <p>Upload complex reports, verify against standards, generate requirements, and capture component parameters.</p>
</div>
""", unsafe_allow_html=True)
...
KEYWORD_TO_STANDARD_MAP = {
    # Connectivity
    "gps": "NMEA 0183 / GNSS Performance Standards",
    "gnss": "3GPP / GNSS Performance Standards",
    "bluetooth": "Bluetooth Core Specification",
    "wifi": "IEEE 802.11 Standards",
    "wi-fi": "IEEE 802.11 Standards",
    "lte": "3GPP LTE Standards",
    "4g": "3GPP LTE Standards",
    "sim": "ISO/IEC 7816",
    "can": "ISO 11898",
    "usb": "USB-IF Standards",
    # Sensors
    "sensor": "AEC-Q104 (Sensors)",
    "gyro": "AEC-Q104 (Sensors)",
    "accelero": "AEC-Q104 (Sensors)",
    "magneto": "AEC-Q104 (Sensors)",
    "temp": "System Thermal Design Spec",
    # Software & Stability
    "touch": "HMI/Driver Interface Spec",
    "display": "Display Quality Standards",
    "rgb": "Display Quality Standards",
    "crash": "System Stability/Software Quality Standard",
    "anr": "System Stability/Software Quality Standard",
    "watchdog": "System Watchdog Functionality Spec",
    "rtc": "System Real-Time Clock Spec",
    "memory": "Embedded System Memory Management",
    "modem": "3GPP Modem Interface Standards",
    # E-Bike Specific
    "ip rating": "IEC 60529",
    "short circuit": "AIS-156 / IEC 62133",
    "overcharge": "AIS-156 / ISO 12405-4",
    "over-discharge": "AIS-156 / ISO 12405-4",
    "vibration": "IEC 60068-2-6 / AIS-048",
    "fatigue": "ISO 4210-6",
    "braking": "EN 15194 / ISO 4210-2",
    "emc": "IEC 61000 / EN 15194"
}
...
# ============= Module: Test Report Verification (UPGRADED) =============
if option == "Test Report Verification":
    ...
    st.caption("Upload PDF/DOCX reports. The parser intelligently extracts and groups tests by PASS/FAIL status.")

    uploaded_file = st.file_uploader("Upload a report file", type=["pdf", "docx", "xlsx"])
    if uploaded_file:
        parsed = parse_report(uploaded_file)
        if parsed:
            # --- Filter tests into groups ---
            failed_tests = [t for t in parsed if t.get("Result", "").upper() == "FAIL"]
            passed_tests = [t for t in parsed if t.get("Result", "").upper() == "PASS"]
            other_tests = [t for t in parsed if t.get("Result", "").upper() not in ["PASS", "FAIL"]]

            # --- Display FAILED tests first and prominently ---
            if failed_tests:
                ...
                st.markdown("---") # Add a separator
                 ...
                 st.success("✅ No failed test cases were found in the report.")

            # --- Display PASSED tests in a collapsible expander ---
            if passed_tests:
                with st.expander(f"✅ View {len(passed_tests)} Passed Test Case(s)", expanded=False):
